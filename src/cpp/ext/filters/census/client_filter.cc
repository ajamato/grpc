...
void CensusClientCallData::Destroy(grpc_call_element* elem,
  ...
  std::vector<std::pair<opencensus::tags::TagKey, std::string>> tags = context_.tags().tags();
  tags.emplace_back(ClientMethodTagKey(), method_);
  tags.emplace_back(ClientStatusTagKey(), StatusCodeToString(final_info->final_status));
  ...
  if (final_info->final_status != GRPC_STATUS_OK) {
    ...
    context_.Span().SetStatus(opencensus::trace::StatusCode::UNKNOWN, StatusCodeToString(final_info->final_status));
  }
  context_.EndSpan();
}
